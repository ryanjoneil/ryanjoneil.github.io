...
# Minimize the sum of squared overbookings.
@objective(m, Min, sum(ci^2 for ci in c))
...
plot(
    ...
    Theme(alphas=[0.5])
) |> SVG("minimize-overbooking-squared-teams.svg", 8inch, 4inch)
...
plot(
    ...
    Scale.color_continuous,
    Theme(alphas=[0.5])
) |> SVG("minimize-overbooking-squared-managers.svg", 8inch, 4inch)
